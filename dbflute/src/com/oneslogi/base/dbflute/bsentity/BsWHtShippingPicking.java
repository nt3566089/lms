 ...
 *
 * [column]
 *     HT_SHIPPING_PICKING_ID, MAC_ADDRESS, CLIENT_ID, CENTER_ID, PICKING_B_ID, NIZOROE_NO, PRODUCT_ID, JAN_CD, PRODUCT_CD, PRODUCT_NM, INST_NUM, SPG_QTY_ONS, LOCATION_ID, LOCATION_CD, LOCATION_NM, LOT, LIMIT_DT, WORK_START_DT, WORK_END_DT, STORE_LABEL_NO, LOT_ID, PICKING_FLG, SKIP_NUM, PICKING_WORK_NO, PROD_DT, FORCE_COPLETE_FLG, DAIHYO_NIZOROE_NO, NIZOROE_NO2, DAIHYO_NIZOROE_NO2, SUSPEND_FLG, WORK_GROUP_KEY, ANOTHER_PICKING_FLG, LAP_NUM, MULTI_PICK_FLG, MULTI_MAC_ADDRESS, DEL_FLG, VERSION_NO, CONTROL_NO, ADD_DT, ADD_USER, ADD_PROCESS, UPD_DT, UPD_USER, UPD_PROCESS
 *
 * [sequence]
 *     
 *
 ...
 * String multiPickFlg = entity.getMultiPickFlg();
 * String multiMacAddress = entity.getMultiMacAddress();
 ...
 * entity.setMultiPickFlg(multiPickFlg);
 * entity.setMultiMacAddress(multiMacAddress);
...
public abstract class BsWHtShippingPicking extends AbstractEntity implements DomainEntity, EntityDefinedCommonColumn {
    ...
    protected String _multiPickFlg;

    /** MULTI_MAC_ADDRESS: {VARCHAR(30)} */
    protected String _multiMacAddress;
    ...
    protected String doBuildColumnString(String dm) {
        ...
        sb.append(dm).append(xfND(_multiPickFlg));
        sb.append(dm).append(xfND(_multiMacAddress));
    ...
    //                                                                            ========
    /**
     * [get] HT_SHIPPING_PICKING_ID: {PK, ID, NotNull, BIGINT(19)} <br>
     * 出荷ピッキングワークID
    ...
    /**
     * [set] HT_SHIPPING_PICKING_ID: {PK, ID, NotNull, BIGINT(19)} <br>
     * 出荷ピッキングワークID
    ...
    /**
     * [get] MULTI_PICK_FLG: {CHAR(1)} <br>
     * 複数人ピッキングフラグ
     * @return The value of the column 'MULTI_PICK_FLG'. (NullAllowed even if selected: for no constraint)
     */
    public String getMultiPickFlg() {
        checkSpecifiedProperty("multiPickFlg");
        return convertEmptyToNull(_multiPickFlg);
    }

    /**
     * [set] MULTI_PICK_FLG: {CHAR(1)} <br>
     * 複数人ピッキングフラグ
     * @param multiPickFlg The value of the column 'MULTI_PICK_FLG'. (NullAllowed: null update allowed for no constraint)
     */
    public void setMultiPickFlg(String multiPickFlg) {
        registerModifiedProperty("multiPickFlg");
        _multiPickFlg = multiPickFlg;
    }

    /**
     * [get] MULTI_MAC_ADDRESS: {VARCHAR(30)} <br>
     * MACアドレス(複数人作業者)
     * @return The value of the column 'MULTI_MAC_ADDRESS'. (NullAllowed even if selected: for no constraint)
     */
    public String getMultiMacAddress() {
        checkSpecifiedProperty("multiMacAddress");
        return convertEmptyToNull(_multiMacAddress);
    }

    /**
     * [set] MULTI_MAC_ADDRESS: {VARCHAR(30)} <br>
     * MACアドレス(複数人作業者)
     * @param multiMacAddress The value of the column 'MULTI_MAC_ADDRESS'. (NullAllowed: null update allowed for no constraint)
     */
    public void setMultiMacAddress(String multiMacAddress) {
        registerModifiedProperty("multiMacAddress");
        _multiMacAddress = multiMacAddress;
    }
