...
public abstract class BsTShippingInstHDtoMapper implements DtoMapper<TShippingInstH, TShippingInstHDto>, Serializable {
    ...
    protected boolean _suppressMSalesOrgBySalesOrg;
    protected boolean _suppressMCustomerByAccoutId;
    ...
    public TShippingInstHDto mappingToDto(TShippingInstH entity) {
        ...
        if (!_suppressMSalesOrgBySalesOrg && entity.getMSalesOrgBySalesOrg() != null) {
            MSalesOrg relationEntity = entity.getMSalesOrgBySalesOrg();
            Entity relationKey = createInstanceKeyEntity(relationEntity);
            Object cachedDto = instanceCache ? _relationDtoMap.get(relationKey) : null;
            if (cachedDto != null) {
                MSalesOrgDto relationDto = (MSalesOrgDto)cachedDto;
                dto.setMSalesOrgBySalesOrg(relationDto);
                if (reverseReference) {
                    relationDto.getTShippingInstHBySalesOrgList().add(dto);
                }
            } else {
                ...
                mapper.setReverseReference(reverseReference);
                if (!instanceCache) { mapper.disableInstanceCache(); }
                mapper.suppressTShippingInstHBySalesOrgList();
                ...
                if (instanceCache && relationEntity.hasPrimaryKeyValue()) {
                    _relationDtoMap.put(relationKey, dto.getMSalesOrgBySalesOrg());
                }
            }
        };
        if (!_suppressMCustomerByAccoutId && entity.getMCustomerByAccoutId() != null) {
            MCustomer relationEntity = entity.getMCustomerByAccoutId();
            Entity relationKey = createInstanceKeyEntity(relationEntity);
            Object cachedDto = instanceCache ? _relationDtoMap.get(relationKey) : null;
            if (cachedDto != null) {
                MCustomerDto relationDto = (MCustomerDto)cachedDto;
                dto.setMCustomerByAccoutId(relationDto);
                if (reverseReference) {
                    relationDto.getTShippingInstHByAccoutIdList().add(dto);
                }
            } else {
                MCustomerDtoMapper mapper = new MCustomerDtoMapper(_relationDtoMap, _relationEntityMap);
                mapper.setExceptCommonColumn(exceptCommonColumn);
                mapper.setReverseReference(reverseReference);
                if (!instanceCache) { mapper.disableInstanceCache(); }
                mapper.suppressTShippingInstHByAccoutIdList();
                MCustomerDto relationDto = mapper.mappingToDto(relationEntity);
                dto.setMCustomerByAccoutId(relationDto);
                if (reverseReference) {
                    relationDto.getTShippingInstHByAccoutIdList().add(dto);
                ...
                if (instanceCache && relationEntity.hasPrimaryKeyValue()) {
                    _relationDtoMap.put(relationKey, dto.getMCustomerByAccoutId());
                }
            }
        };
    ...
    public TShippingInstH mappingToEntity(TShippingInstHDto dto) {
        ...
        if (!_suppressMSalesOrgBySalesOrg && dto.getMSalesOrgBySalesOrg() != null) {
            MSalesOrgDto relationDto = dto.getMSalesOrgBySalesOrg();
            Object relationKey = createInstanceKeyDto(relationDto, relationDto.instanceHash());
            Entity cachedEntity = instanceCache ? _relationEntityMap.get(relationKey) : null;
            if (cachedEntity != null) {
                MSalesOrg relationEntity = (MSalesOrg)cachedEntity;
                entity.setMSalesOrgBySalesOrg(relationEntity);
                if (reverseReference) {
                    relationEntity.getTShippingInstHBySalesOrgList().add(entity);
                }
            } else {
                MSalesOrgDtoMapper mapper = new MSalesOrgDtoMapper(_relationDtoMap, _relationEntityMap);
                mapper.setExceptCommonColumn(exceptCommonColumn);
                ...
                if (instanceCache && entity.getMSalesOrgBySalesOrg().hasPrimaryKeyValue()) {
                    _relationEntityMap.put(relationKey, entity.getMSalesOrgBySalesOrg());
                }
            }
        };
        if (!_suppressMCustomerByAccoutId && dto.getMCustomerByAccoutId() != null) {
            MCustomerDto relationDto = dto.getMCustomerByAccoutId();
            Object relationKey = createInstanceKeyDto(relationDto, relationDto.instanceHash());
            Entity cachedEntity = instanceCache ? _relationEntityMap.get(relationKey) : null;
            if (cachedEntity != null) {
                MCustomer relationEntity = (MCustomer)cachedEntity;
                entity.setMCustomerByAccoutId(relationEntity);
                if (reverseReference) {
                    relationEntity.getTShippingInstHByAccoutIdList().add(entity);
                }
            } else {
                MCustomerDtoMapper mapper = new MCustomerDtoMapper(_relationDtoMap, _relationEntityMap);
                mapper.setExceptCommonColumn(exceptCommonColumn);
                ...
                if (instanceCache && entity.getMCustomerByAccoutId().hasPrimaryKeyValue()) {
                    _relationEntityMap.put(relationKey, entity.getMCustomerByAccoutId());
                }
            }
        };
    ...
    public void suppressMSalesOrgBySalesOrg() {
        _suppressMSalesOrgBySalesOrg = true;
    }
    public void suppressMCustomerByAccoutId() {
        _suppressMCustomerByAccoutId = true;
    }
    ...
    protected void doSuppressAll() { // internal
        ...
        suppressMSalesOrgBySalesOrg();
        suppressMCustomerByAccoutId();
    ...
    protected void doSuppressClear() { // internal
        ...
        _suppressMSalesOrgBySalesOrg = false;
        _suppressMCustomerByAccoutId = false;
