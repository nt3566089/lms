...
public abstract class BsMCustomerDtoMapper implements DtoMapper<MCustomer, MCustomerDto>, Serializable {
    ...
    protected boolean _suppressMPlantStorageSpaceByCd;
    ...
    protected boolean _suppressTShippingInstHByAccoutIdList;
    ...
    public MCustomerDto mappingToDto(MCustomer entity) {
        ...
        if (!_suppressMPlantStorageSpaceByCd && entity.getMPlantStorageSpaceByCd() != null) {
            MPlantStorageSpace relationEntity = entity.getMPlantStorageSpaceByCd();
            Entity relationKey = createInstanceKeyEntity(relationEntity);
            Object cachedDto = instanceCache ? _relationDtoMap.get(relationKey) : null;
            if (cachedDto != null) {
                MPlantStorageSpaceDto relationDto = (MPlantStorageSpaceDto)cachedDto;
                dto.setMPlantStorageSpaceByCd(relationDto);
                if (reverseReference) {
                }
            } else {
                MPlantStorageSpaceDtoMapper mapper = new MPlantStorageSpaceDtoMapper(_relationDtoMap, _relationEntityMap);
                mapper.setExceptCommonColumn(exceptCommonColumn);
                mapper.setReverseReference(reverseReference);
                if (!instanceCache) { mapper.disableInstanceCache(); }
                MPlantStorageSpaceDto relationDto = mapper.mappingToDto(relationEntity);
                dto.setMPlantStorageSpaceByCd(relationDto);
                if (reverseReference) {
                }
                if (instanceCache && relationEntity.hasPrimaryKeyValue()) {
                    _relationDtoMap.put(relationKey, dto.getMPlantStorageSpaceByCd());
                }
            }
        };
        ...
        if (!_suppressTShippingInstHByAccoutIdList && !entity.getTShippingInstHByAccoutIdList().isEmpty()) {
            TShippingInstHDtoMapper mapper = new TShippingInstHDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressMCustomerByAccoutId();
            List<TShippingInstHDto> relationDtoList = mapper.mappingToDtoList(entity.getTShippingInstHByAccoutIdList());
            dto.setTShippingInstHByAccoutIdList(relationDtoList);
            if (reverseReference) {
                for (TShippingInstHDto relationDto : relationDtoList) {
                    relationDto.setMCustomerByAccoutId(dto);
                }
            }
        };
        return dto;
    ...
    public MCustomer mappingToEntity(MCustomerDto dto) {
        ...
        if (!_suppressMPlantStorageSpaceByCd && dto.getMPlantStorageSpaceByCd() != null) {
            MPlantStorageSpaceDto relationDto = dto.getMPlantStorageSpaceByCd();
            Object relationKey = createInstanceKeyDto(relationDto, relationDto.instanceHash());
            Entity cachedEntity = instanceCache ? _relationEntityMap.get(relationKey) : null;
            if (cachedEntity != null) {
                MPlantStorageSpace relationEntity = (MPlantStorageSpace)cachedEntity;
                entity.setMPlantStorageSpaceByCd(relationEntity);
                if (reverseReference) {
                }
            } else {
                MPlantStorageSpaceDtoMapper mapper = new MPlantStorageSpaceDtoMapper(_relationDtoMap, _relationEntityMap);
                ...
                mapper.setReverseReference(reverseReference);
                if (!instanceCache) { mapper.disableInstanceCache(); }
                MPlantStorageSpace relationEntity = mapper.mappingToEntity(relationDto);
                entity.setMPlantStorageSpaceByCd(relationEntity);
                if (reverseReference) {
                }
                if (instanceCache && entity.getMPlantStorageSpaceByCd().hasPrimaryKeyValue()) {
                    _relationEntityMap.put(relationKey, entity.getMPlantStorageSpaceByCd());
                }
            }
        };
        ...
        if (!_suppressTShippingInstHByAccoutIdList && !dto.getTShippingInstHByAccoutIdList().isEmpty()) {
            TShippingInstHDtoMapper mapper = new TShippingInstHDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressMCustomerByAccoutId();
            List<TShippingInstH> relationEntityList = mapper.mappingToEntityList(dto.getTShippingInstHByAccoutIdList());
            entity.setTShippingInstHByAccoutIdList(relationEntityList);
            if (reverseReference) {
                for (TShippingInstH relationEntity : relationEntityList) {
                    relationEntity.setMCustomerByAccoutId(entity);
                }
            }
        };
        return entity;
    ...
    public void suppressMPlantStorageSpaceByCd() {
        _suppressMPlantStorageSpaceByCd = true;
    }
    ...
    public void suppressTShippingInstHByAccoutIdList() {
        _suppressTShippingInstHByAccoutIdList = true;
    }
    protected void doSuppressAll() { // internal
        suppressMClient();
        suppressMCustomerSelf();
        suppressMClientCenterByClientId();
        suppressMPlantStorageSpaceByCd();
    ...
    protected void doSuppressClear() { // internal
        ...
        _suppressTShippingInstHByAccoutIdList = false;
    }
