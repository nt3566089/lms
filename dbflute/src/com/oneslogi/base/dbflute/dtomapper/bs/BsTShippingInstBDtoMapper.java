...
public abstract class BsTShippingInstBDtoMapper implements DtoMapper<TShippingInstB, TShippingInstBDto>, Serializable {
    ...
    protected boolean _suppressTShippingInstBPrintList;
    protected boolean _suppressTShippingInstDataTransferList;
    ...
    public TShippingInstBDto mappingToDto(TShippingInstB entity) {
        ...
        dto.setUs3fTypeCd(entity.getUs3fTypeCd());
        ...
        if (!_suppressTShippingInstBPrintList && !entity.getTShippingInstBPrintList().isEmpty()) {
            TShippingInstBPrintDtoMapper mapper = new TShippingInstBPrintDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressTShippingInstB();
            List<TShippingInstBPrintDto> relationDtoList = mapper.mappingToDtoList(entity.getTShippingInstBPrintList());
            dto.setTShippingInstBPrintList(relationDtoList);
            if (reverseReference) {
                for (TShippingInstBPrintDto relationDto : relationDtoList) {
                    relationDto.setTShippingInstB(dto);
                }
            }
        };
        if (!_suppressTShippingInstDataTransferList && !entity.getTShippingInstDataTransferList().isEmpty()) {
            TShippingInstDataTransferDtoMapper mapper = new TShippingInstDataTransferDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressTShippingInstB();
            List<TShippingInstDataTransferDto> relationDtoList = mapper.mappingToDtoList(entity.getTShippingInstDataTransferList());
            dto.setTShippingInstDataTransferList(relationDtoList);
            if (reverseReference) {
                for (TShippingInstDataTransferDto relationDto : relationDtoList) {
                    relationDto.setTShippingInstB(dto);
                }
            }
        };
    ...
    public TShippingInstB mappingToEntity(TShippingInstBDto dto) {
        ...
        if (needsMapping(dto, dto.getTaxRate(), "taxRate")) {
        ...
        if (needsMapping(dto, dto.getUs3fTypeCd(), "us3fTypeCd")) {
            entity.setUs3fTypeCd(dto.getUs3fTypeCd());
        }
        ...
        if (!_suppressTShippingInstBPrintList && !dto.getTShippingInstBPrintList().isEmpty()) {
            TShippingInstBPrintDtoMapper mapper = new TShippingInstBPrintDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressTShippingInstB();
            List<TShippingInstBPrint> relationEntityList = mapper.mappingToEntityList(dto.getTShippingInstBPrintList());
            entity.setTShippingInstBPrintList(relationEntityList);
            if (reverseReference) {
                for (TShippingInstBPrint relationEntity : relationEntityList) {
                    relationEntity.setTShippingInstB(entity);
                }
            }
        };
        if (!_suppressTShippingInstDataTransferList && !dto.getTShippingInstDataTransferList().isEmpty()) {
            TShippingInstDataTransferDtoMapper mapper = new TShippingInstDataTransferDtoMapper(_relationDtoMap, _relationEntityMap);
            mapper.setExceptCommonColumn(exceptCommonColumn);
            mapper.setReverseReference(reverseReference);
            if (!instanceCache) { mapper.disableInstanceCache(); }
            mapper.suppressTShippingInstB();
            List<TShippingInstDataTransfer> relationEntityList = mapper.mappingToEntityList(dto.getTShippingInstDataTransferList());
            entity.setTShippingInstDataTransferList(relationEntityList);
            if (reverseReference) {
                for (TShippingInstDataTransfer relationEntity : relationEntityList) {
                    relationEntity.setTShippingInstB(entity);
                }
            }
        };
    ...
    public void suppressTShippingInstBPrintList() {
        _suppressTShippingInstBPrintList = true;
    }
    public void suppressTShippingInstDataTransferList() {
        _suppressTShippingInstDataTransferList = true;
    }
    ...
    protected void doSuppressAll() { // internal
        ...
        suppressTShippingInstBPrintList();
        suppressTShippingInstDataTransferList();
    ...
    protected void doSuppressClear() { // internal
        ...
        _suppressTShippingInstBPrintList = false;
        _suppressTShippingInstDataTransferList = false;
