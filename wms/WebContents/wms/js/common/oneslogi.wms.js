...
//				},0);
		...
		modalJan: function(items){
			var elmFocus = document.activeElement;
			var modal = $modal.open({
				templateUrl:'wms/partials/assist/ProductMasterJanSearch.html',
				controller:'ProductMasterJanSearch',
				//[検査-170] 画面外をクリックした時に、自動で画面が閉じ無い様に修正します。 2014.12.01 KI Start
        		backdrop:'static',
        		//[検査-170] 画面外をクリックした時に、自動で画面が閉じ無い様に修正します。 2014.12.01 KI End
				resolve: {
					getScreen: ['owsScreen', function(owsScreen) {
						return new owsScreen.getData('ProductMasterJanSearch', true);
					}],
					getItem: ['owsItem', function(owsItem) {
						return new owsItem.getData('ProductMasterJanSearch');
					}],
					getColumn: ['owsColumn', function(owsColumn) {
						return new owsColumn.getData('ProductMasterJanSearch');
					}],
					items: function () {
						return items;
					}
				...
				windowClass: 'app-modal-window',
				beforeClose: function() {
					alertMessage.removeModalAlert('ProductMasterJanSearch');
					elmFocus.focus();
					getNextObject(elmFocus, true).focus();
				},
				beforeDismiss: function() {
			...
			modal.opened.then(function() {
				alertMessage.addModalAlert(modal, 'ProductMasterJanSearch');
//				$timeout(function(){
//	                if ($(".modal-dialog").length) {
//	                	$(".modal-dialog").draggable();
//	                }
