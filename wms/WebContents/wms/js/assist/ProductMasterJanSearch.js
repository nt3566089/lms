 ...
 * 商品マスタ検索画面<br>
 */
angular.module('oneslogiWms.ProductMasterJanSearch', [])
 ...
 *
 */
.factory('productMasterJanSearchApi', ['AngularAPIClient', function(AngularAPIClient) {
	return {
		...
		productMasterSearch: AngularAPIClient.make(
				[
				 ...
				 ['search', 'GET',  'resources/assist/productMasterSearch/JanSearch']
				 ]
		)
	};
}])
 ...
 * 当画面の制御ロジックはここに実装される。
 */
.controller('ProductMasterJanSearch', ['$scope', '$modalInstance','$route', '$filter', '$location', '$q', 'alertMessage', 'directiveControl', 'gridDirectiveControl', 'optionMessageControl', 'gridOptionMessageControl', 'owsHistory', 'owsCommon', 'userInfo', 'statusInfo', 'modalFactory', 'productMasterSearchApi',"items", function($scope,$modalInstance, $route, $filter, $location, $q, alertMessage, directiveControl, gridDirectiveControl, optionMessageControl, gridOptionMessageControl, owsHistory, owsCommon, userInfo, statusInfo, modalFactory, api,items){
	...
	$scope.deferredGetInitData = function () {
		...
		api.productMasterSearch.initNew().then(function(response){
			...
			// 処理結果確認
			if (!statusInfo.isSuccessAndShowMessage(response, 'productMasterJanSearchform')) {
				deferred.reject();
				return;
			}
			...
			if (items) {
				...
				$scope.productSearch.janCd = items.janCd;
				editableFlg = items.editableFlg;
			}

			$scope.editableFlg = editableFlg;

			owsCommon.setFirstFocus("productMasterJanSearchform");
	...
	$scope.getPagedDataAsync = function(pageSize, page) {
		...
		api.productMasterSearch.search(request).then(function(response) {
			...
			statusInfo.isSuccessAndShowMessage(response, 'productMasterJanSearchform');
