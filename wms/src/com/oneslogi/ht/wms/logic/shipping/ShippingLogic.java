...
//		clearWHtShippingPicking(wHtShippingPicking);
	...
	public void clearInspectionFlg(WHtShipping wHtShipping, PieceCaseCls pieceCaseCls) {
		...
		packingBCb.query().queryTPickingB().queryTPickingH().setClientId_Equal(wHtShipping.getClientId());
		packingBCb.query().queryTPickingB().queryTPickingH().setPickingWorkNo_Equal(wHtShipping.getPickingWorkNo());
		// [#18787] 出荷確定後に特殊操作で確定前ステータスに戻る不具合を修正 2022.04.15 takahashi Start
		// 確定済みを除くための条件
		// ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
		packingBCb.query().queryTPickingB().queryTPickingH().setPickingStatus_NotEqual_$55();
		// [#18787] 出荷確定後に特殊操作で確定前ステータスに戻る不具合を修正 2022.04.15 takahashi End
		...
		// [#18787] 出荷確定後に特殊操作で確定前ステータスに戻る不具合を修正 2022.04.15 takahashi Start
		// 出庫ボディ 更新実行
		int updCount = tPackingBBhv.queryUpdate(updatePackingB, packingBCb);
		// 更新処理を無視する
		if(updCount == 0) {
			return;
		}
	...
	public void clearInspectionFlgByPackingNo(WHtShipping wHtShipping){
		...
		packingBCb.query().queryTPickingB().queryTPickingH().setClientId_Equal(wHtShipping.getClientId());
		packingBCb.query().queryTPackingH().setShippingPackingNo_Equal(wHtShipping.getPickingWorkNo());
		// [#18787] 出荷確定後に特殊操作で確定前ステータスに戻る不具合を修正 2022.04.20 takahashi Start
		// ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
		packingBCb.query().queryTPackingH().setPackingStatus_NotEqual_$55();
		...
		// [#18787] 出荷確定後に特殊操作で確定前ステータスに戻る不具合を修正 2022.04.20 takahashi End
...
//			}
	...
	// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.19 takahashi Start
	/**
	 * <h2>出荷ステータスが確定済チェックを行う。</h2>
	 * <pre>
	 * 作業中の出荷ステータスが確定済に変更されているかチェックを行う。
	 * @param wHtShipping
	 * @return true: 確定済でない, false: 確定済に対して作業中
	 */
	public boolean checkUpdateShippingInspectStatus(WHtShipping wHtShipping) {
		final long centerId = wHtShipping.getCenterId();
		final String targetNo = wHtShipping.getPickingWorkNo();
		...
		if (commonLogic.isPickingWorkNo(centerId, targetNo)) {
			...
			tPickingBCB.query().queryTPickingH().setClientId_Equal(wHtShipping.getClientId());
			tPickingBCB.query().queryTPickingH().setPickingWorkNo_Equal(wHtShipping.getPickingWorkNo());
			// 検索対象からステータス55:出荷確定済を対象外
			// ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
			tPickingBCB.query().queryTPickingH().setPickingStatus_NotEqual_$55();
			...
			// 検索対象からステータス55:出荷確定済を対象外
			// ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
			tPackingBCB.query().queryTPackingH().setPackingStatus_NotEqual_$55();
	...
	// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.19 takahashi End
