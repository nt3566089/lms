...
import com.oneslogi.base.dbflute.cbean.MLocationReplenishProductCB;
...
public class ProductMasterSelectLogic extends AbstractWmsLogic {
	...
	public PagingResultBean<MProduct> selectByConditions(MProduct header, PagingData paging, ErrorStatus errSts) {
		...
		//製造日管理フラグ
		cb.setupSelect_BClassDtlByProdDtManagFlg();
		cb.setupSelect_BClassDtlByProdDtManagFlg().withVDict(getCultureId());

		//物流特殊区分
		cb.setupSelect_BClassDtlByLogiSpecialTypeCd();
		cb.setupSelect_BClassDtlByLogiSpecialTypeCd().withVDict(getCultureId());

		//OCR日付形式
		cb.setupSelect_BClassDtlByOcrDateFormat();
		cb.setupSelect_BClassDtlByOcrDateFormat().withVDict(getCultureId());

		//沖縄航空便不可フラグ
		cb.setupSelect_BClassDtlByNoOkinawaFlightFlg();
		cb.setupSelect_BClassDtlByNoOkinawaFlightFlg().withVDict(getCultureId());

		//物流注意区分
		cb.setupSelect_BClassDtlByLogiAttentionTypeCd();
		cb.setupSelect_BClassDtlByLogiAttentionTypeCd().withVDict(getCultureId());

		//重量フラグ
		cb.setupSelect_BClassDtlByHeavyItemFlg();
		cb.setupSelect_BClassDtlByHeavyItemFlg().withVDict(getCultureId());

		//保険品フラグ
		cb.setupSelect_BClassDtlByInsuranceFlg();
		cb.setupSelect_BClassDtlByInsuranceFlg().withVDict(getCultureId());

		//大物フラグ
		cb.setupSelect_BClassDtlByLargeItemFlg();
		cb.setupSelect_BClassDtlByLargeItemFlg().withVDict(getCultureId());
		...
		mProductBhv.loadMProductShapeList(result, new ConditionBeanSetupper<MProductShapeCB>() {
			public void setup(MProductShapeCB subCB) {
				...
				subCB.setupSelect_MShapeGrpDtl().withMShape();//荷姿グループマスタ明細・荷姿マスタ
		...
		// ミドリ安全 商品マスタ修正 2024.05.20 Jeongmin [S]
		// 商品に紐付くロケーション補充商品マスタ(ロケーションマスタ関連含む)を抽出
		mProductBhv.loadMLocationReplenishProductList(result, new ConditionBeanSetupper<MLocationReplenishProductCB>() {
			public void setup(MLocationReplenishProductCB subCB) {
				subCB.setupSelect_MProduct().withMClient();//商品マスタ・荷主マスタ
				subCB.setupSelect_MLocation().withBClassDtlByLocationType().withVDict(getCultureId());;//ロケーションマスタ
				subCB.query().setDelFlg_Equal_$0();
			}
		});
		// ミドリ安全 商品マスタ修正 2024.05.20 Jeongmin [E]
