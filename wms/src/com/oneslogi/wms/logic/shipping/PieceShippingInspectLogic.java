...
//[C-CWMS-0049]商品荷姿マスタの荷姿入数の取得方法修正  2016/1/13 chou Start
	...
	public void tPickingRUpdate(TPickingR update, TPickingH tPickingH, ErrorStatus errSts) {
		...
		// ミドリ安全様向けカスタマイズ対応 検索条件修正 2024.7.15 張 Mod [S]
...
//		cb.query().queryTPickingH().setPickingWorkNo_Equal(tPickingH.getPickingWorkNo());
		cb.query().queryTPickingH().setPickingHId_Equal(tPickingH.getPickingHId());
		// ミドリ安全様向けカスタマイズ対応 検索条件修正 2024.7.15 張 Mod [E]
	...
	// ミドリ安全様向けカスタマイズ対応 コントロールNo.更新ロジック追加  2024.7.15 張 Add [S]
	/**
	 * <h2>コントロールNo.のクリア</h2>
	 * @param controlNo：コントロールNo.
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void clearControlNoWithControlNo(Long controlNo, ErrorStatus errSts) {
	 ...
	 */
	public void updateTPickingRPillOutFlg(Long controlNo, ErrorStatus errStatus) {
		//更新処理
		TPickingR tPickingR = new TPickingR();
		tPickingR.setPllOutFlg("1");
		Timestamp time = new Timestamp(System.currentTimeMillis());
		tPickingR.setPllOutUserId(getUserId());
		tPickingR.setPllOutDt(time);
		tPickingR.setControlNo(null);
	 ...
	 * @param errStatus エラー時に設定するエラーステータス
	 */
	public void updateTPickingRTagOutFlg(Long controlNo, ErrorStatus errStatus) {
		//更新処理
		TPickingR tPickingR = new TPickingR();
		tPickingR.setTagOutFlg("1");
		Timestamp time = new Timestamp(System.currentTimeMillis());
		tPickingR.setTagOutUserId(getUserId());
		tPickingR.setTagOutDt(time);
		tPickingR.setControlNo(null);
	...
	// ミドリ安全様向けカスタマイズ対応 コントロールNo.更新ロジック追加  2024.7.15 張 Add [E]
}
