...
//	private ParamLogic paramLogic;
	 ...
	 */
	public UpdateInfo update(TPackingH tPackingH, TAllocInstH tAllocInstH, List<SqlPackingListUpdate> inputPackingUpdateList, ErrorStatus errSts) {

		// ===== 差分の抜出 =====

		// 変更前の情報をDBから検索
		List<SqlPackingListUpdate> dbPackingUpdateList = selectLogic.select(tPackingH, new PagingData(), errSts);
		if (0 < getErrorManager().size()) {
			return null;
		}
		...
		// ミドリ安全様向けカスタマイズ対応  出庫ヘッダーid取得削除 2024.7.15 張 Del [S]
		// [#5131][v3.1] 種蒔きNo.をピッキンググループNo.単位に変更 2018.08.24 kawana Start
		//final TPickingH dbPickingH = selectPickingHByPickingWorkNo(tPackingH.getCenterId(), tPackingH.getPickingWorkNo());
		//final long pickingHId = dbPickingH.getPickingHId();
		// [#5131][v3.1] 種蒔きNo.をピッキンググループNo.単位に変更 2018.08.24 kawana End
		// ミドリ安全様向けカスタマイズ対応  出庫ヘッダーid取得削除 2024.7.15 張 Del [E]
		
		// ミドリ安全様向けカスタマイズ対応  出庫ヘッダーid取得追加 2024.7.15 張 Add [S]
		List<TPackingH> tPackingHList = selectPickingHByPacking(tPackingH);
		Long pickingHId = tPackingHList.get(0).getPickingHId();
		// ミドリ安全様向けカスタマイズ対応  出庫ヘッダーid取得追加 2024.7.15 張 Add [E]

		// [#2874] HT/SDケースピッキングなしの場合にステータス不正が発生 2018.06.26 kawana Start
		
		// ミドリ安全様向けカスタマイズ対応  HT/SDケースピッキング削除 2024.7.15 張 Del [S]
		//MParam param = null;
...
//			mClientCenter.setClientId(tPackingH.getClientId());
//			mClientCenter.setCenterId(tPackingH.getCenterId());
//			param = paramLogic.getUkEntityWithDeletedCheck(mClientCenter);
			// ミドリ安全様向けカスタマイズ対応  HT/SDケースピッキング削除 2024.7.15 張 Del [S]
//			// 出庫フラグ・検品フラグの状態を取得
			//pickingFlgInfo = getPickingFlgInfo(pickingHId, param);
			// ミドリ安全様向けカスタマイズ対応  HT/SDケースピッキング削除 2024.7.15 張 Del [E]
		//}
	// ミドリ安全様向けカスタマイズ対応  HT/SDケースピッキング削除 2024.7.15 張 Del [E]
		// [#2874] HT/SDケースピッキングなしの場合にステータス不正が発生 2018.06.26 kawana End
		...
		for (long deletePackingHId : deletePackingHIdList) {
			// ミドリ安全様向けカスタマイズ対応  ピッキング提案ロジック削除 2024.7.15 張 Del [S]
			// [#5107][v3.1] 最適ピッキング提案 機能追加 2018.09.10 kawana Start
			// ピッキング提案データ削除
			//pickingMethodRecommendationCallLogic.packingDelete(deletePackingHId);
			// [#5107][v3.1] 最適ピッキング提案 機能追加 2018.09.10 kawana End
			// ミドリ安全様向けカスタマイズ対応  ピッキング提案ロジック削除 2024.7.15 張 Del [E]
		...
		// ===== 梱包の変更 =====
		// 変更分の梱包ボディを削除
		Map<Long,List<TPackingB>> updatePackingInfoMap = new HashMap<Long,List<TPackingB>>();
		...
		for (long changePackingHId : changeInputEntityMap.keySet()) {
			...
			updatePackingInfoMap.put(changePackingHId, packingBList);
		...
		for (Map.Entry<Long, List<SqlPackingListUpdate>> inputEntityEntry : changeInputEntityMap.entrySet()) {
			...
			// ミドリ安全様向けカスタマイズ対応  梱包ボディのindex追加 2024.7.15 張 Add [S]
			int index = 0;
			List<TPackingB> packingBList = updatePackingInfoMap.get(packingHId);
			List<Long> dbBIds = packingBList.stream().map(o -> {
				return o.getPackingBId();
			}).collect(Collectors.toList());
//			int inputSize = inputEntityList.size();
//			int dbSize = packingBList.size();
			TPackingB packingB;
			// ミドリ安全様向けカスタマイズ対応  梱包ボディのindex追加 2024.7.15 張 Add [E]
			...
			for (SqlPackingListUpdate inputEntity : inputEntityList) {
				...
				// ミドリ安全様向けカスタマイズ対応  梱包ボディ取得追加 2024.7.15 張 Add [S]
				boolean isnewPackingB = false;
				if(dbBIds.contains(inputEntity.getOldBid())) {
					packingB = updatePackingInfoMap.get(packingHId).get(index);
					index++;
				}else {
					packingB = new TPackingB();
					isnewPackingB = true;
				}
//				if(index < (dbSize < inputSize ? dbSize : inputSize)) {
//					packingB = updatePackingInfoMap.get(packingHId).get(index);
//					index++;
//				}else {
//					packingB = new TPackingB();
...
//				}
				...
				for (Map.Entry<Long, PackingNumInfo> packingInfoEntry : packingNumInfoMap.entrySet()) {
					...
					// ミドリ安全様向けカスタマイズ対応  梱包ボディ作成修正 2024.7.15 張 Mod [S]
					// 梱包ボディ作成
					if(isnewPackingB) {
...
//						packingB.setOldPackingBId(inputEntity.getOldBid());
//						packingB.setInspectionFlg_$1();
//						packingB.setPickingFlg_$1();
//						packingB.setCasePickingNo(inputEntity.getCasePickingNo());
//						packingB.setReceiveLabelNo(inputEntity.getReceiveLabelNo());
						// 登録リストに追加
						insertPackingBList.add(packingB);
					}else {
						TPackingB clone = packingB.clone();
						clone.setPackingNum(newPackingNum);
						clone.setPickingBId(pickingBId);
						clone.setAllocInstBId(allocInstBId);
						clone.setPackingBId(null);
						clone.setVersionNo(null);
						// 登録リストに追加
						insertPackingBList.add(clone);
					}
...
//					packingB.setAllocInstBId(allocInstBId);
//					packingB.setPackingNum(newPackingNum);
					// ミドリ安全様向けカスタマイズ対応  梱包ボディ作成修正 2024.7.15 張 Mod [E]

					// ミドリ安全様向けカスタマイズ対応  出庫フラグ・検品フラグの設定削除 2024.7.15 張 Del [S]
					// 出庫フラグ・検品フラグの設定
					//setPackingBFlg(packingB, pickingFlgInfo);
					// ミドリ安全様向けカスタマイズ対応  出庫フラグ・検品フラグの設定削除 2024.7.15 張 Del [E]
					
					// ミドリ安全様向けカスタマイズ対応  梱包ボディリスト追加削除 2024.7.15 張 Del [S]
					// 登録リストに追加
					//insertPackingBList.add(packingB);
					// ミドリ安全様向けカスタマイズ対応  梱包ボディリスト追加削除 2024.7.15 張 Del [S]
		...
		if (!changeInputEntityMap.isEmpty()) {
			...
			for (TPackingH packingH : dbPackingHList) {
				...
				// ミドリ安全様向けカスタマイズ対応  梱包ヘッダの更新処理修正 2024.7.15 張 Mod [S]
				// [#2874] HT/SDケースピッキングなしの場合にステータス不正が発生 2018.06.26 kawana Start
				...
				ResultUpdatePackingData result = updatePackingDataNew(packingH, errSts);
				// [#2874] HT/SDケースピッキングなしの場合にステータス不正が発生 2018.06.26 kawana End
				// ミドリ安全様向けカスタマイズ対応  梱包ヘッダの更新処理修正 2024.7.15 張 Mod [E]
		...
		// ミドリ安全様向けカスタマイズ対応  種蒔きNo.削除 2024.7.15 張 Del [S]
		// [#5131][v3.1] 種蒔きNo.をピッキンググループNo.単位に変更 2018.08.24 kawana Start
		//Integer seedingNo = null;
		// [#5131][v3.1] 種蒔きNo.をピッキンググループNo.単位に変更 2018.08.24 kawana End
		// ミドリ安全様向けカスタマイズ対応  種蒔きNo.削除 2024.7.15 張 Del [E]
		if (!newEntityMap.isEmpty()) {
			// ミドリ安全様向けカスタマイズ対応  箱Noの追加がある判定追加 2024.7.15 張 Add [S]
			updateInfo.doAddPacking = true;
			// ミドリ安全様向けカスタマイズ対応  箱Noの追加がある判定追加 2024.7.15 張 Add [E]
			updateInfo.doUpdate = true;
			updateInfo.doClearTagOut = true; // 後続処理で荷札が梱包単位かを判定
		...
		for (List<SqlPackingListUpdate> newEntityList : newEntityMap.values()) {
			...
			for (SqlPackingListUpdate newEntity : newEntityList) {
				...
				for (Map.Entry<Long, PackingNumInfo> packingInfoEntry : packingNumInfoMap.entrySet()) {
					...
					pickingB.setPackingBInfos(newEntity);
...
//			}
			...
			// ミドリ安全様向けカスタマイズ対応  登録修正 2024.7.15 張 Mod [S]
			//registerNewPackingData(newPickingBList, box, seedingNo, pickingFlgInfo, errSts);
			registerNewPackingData(newPickingBList, box, null, null, errSts);
			// ミドリ安全様向けカスタマイズ対応  登録修正 2024.7.15 張 Mod [E]
			if (0 < getErrorManager().size()) {
				return null;
			}
		}
		...
		clearOutFlgWithTrackingNumberingUnitCheckNew(pickingHId, updateInfo, allocInstH, errSts);
		if (0 < getErrorManager().size()) {
			return null;
		}
	 ...
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void updateDeliveryInfoNew(TPackingH tPackingH, TAllocInstH tAllocInstH, ErrorStatus errSts) {
		...
		clearOutFlgDeliveryInfoNew(tPackingH, allocInstH, errSts);
		if (0 < getErrorManager().size()) {
			return;
		}
	}
...
//	}
	 ...
	 * 出庫ヘッダIDの取得
	 */
	private List<TPackingH> selectPickingHByPacking(TPackingH tPackingH) {
		...
		ListResultBean<TPackingH> tPackingHList = tPackingHBhv.selectList(cb);

		return tPackingHList;
	}
	...
	private void addPackingNumMap(Map<Long, Map<Long, PackingNumInfo>> deletePackingNumMap, TPackingB packingB) {
		...
		if (pickingMap.containsKey(pickingBId)) {
			...
			pickingMap.get(pickingBId).setPackingB(packingB);
		} else {
			// 新規追加
			PackingNumInfo packingNumInfo = new PackingNumInfo(packingNum, packingB.getAllocInstBId(), packingB);
	 ...
	 * @return DB更新した場合はUPDATE_PACKING_H、DB削除した場合はDELETE_PACKING_Hを返却
	 */
	public ResultUpdatePackingData updatePackingDataNew(TPackingH packingH, ErrorStatus errSts) {
	...
	private void registerNewPackingData(List<TPickingB> pickingBList, MBox box, Integer seedingNo, PickingFlgInfo pickingFlgInfo, ErrorStatus errSts) {
		...
		// ミドリ安全様向けカスタマイズ対応  新規登録セット追加 2024.7.15 張 Add [S]
		TPackingH packingH = packingData.get(0);
		List<TPackingB> packingBList = packingData.get(0).getTPackingBList();
		for(int i = 0; i < packingBList.size(); i++) {
			TPackingB tPackingB = packingBList.get(i);
			SqlPackingListUpdate packingBInfos = pickingBList.get(i).getPackingBInfos();
			tPackingB.setOldPackingBId(packingBInfos.getOldBid());
			tPackingB.setReceiveLabelNo(packingBInfos.getReceiveLabelNo());
			tPackingB.setCasePickingNo(packingBInfos.getCasePickingNo());
			tPackingB.setInspectionFlg_$1();
			tPackingB.setPickingFlg_$1();
		}
		SqlPackingListUpdate pinfo = pickingBList.get(0).getPackingBInfos();
		packingH.setPackingStatus(pinfo.getPackingStatus());
		packingH.setCarrierTraceNum(pinfo.getCarrierTraceNum());
		packingH.setNizoroeNo(pinfo.getNizoroeNo());
		packingH.setNizoroeNo2(pinfo.getNizoroeNo2());
		packingH.setDaihyoNizoroeNo(pinfo.getDaihyoNizoroeNo());
		packingH.setDaihyoNizoroeNo2(pinfo.getDaihyoNizoroeNo2());
		packingH.setProcessTypeId(pinfo.getProcessTypeId());
		// ミドリ安全様向けカスタマイズ対応  新規登録セット追加 2024.7.15 張 Add [S]
...
//		}
	...
	private void insertNewPackingData(List<TPackingH> packingHList, PickingFlgInfo pickingFlgInfo) {
		...
		for (TPackingH packingH : packingHList) {
			...
			packingR.setTagOutFlg_$0();
			packingR.setInspectionOutFlg_$0();
			packingR.setCdlOutFlg("0");
			...
			for (TPackingB packingB : packingBList) {
				...
				// 出庫フラグ・検品フラグの設定
				// ミドリ安全様向けカスタマイズ対応  出庫フラグ・検品フラグの設定修正 2024.7.15 張 Del [E]
				//setPackingBFlg(packingB, pickingFlgInfo);
				// ミドリ安全様向けカスタマイズ対応  出庫フラグ・検品フラグの設定修正 2024.7.15 張 Del [E]
	 ...
	 * 帳票の出力状況をクリア(届先変更用)
	 */
	private void clearOutFlgDeliveryInfoNew(TPackingH tPackingH, TAllocInstH allocInstH, ErrorStatus errSts) {
		UpdateInfo updateInfo = new UpdateInfo();
		updateInfo.doUpdate = true;
		updateInfo.doClearSlipOut = true;
		updateInfo.doClearInvoiceCreate = true;
		...
		// 出力フラグのクリア実行
		clearOutFlgNew(tPackingH.getPickingHId(), updateInfo, allocInstH, errSts);
		if (0 < getErrorManager().size()) {
			return;
		}
	}
...
//	}
	 ...
	 *
	 * </pre>
	 * @param pickingHIds 出庫ヘッダIDリスト
	 * @param updateInfo 更新情報
	 * @param allocInstH 引当指示ヘッダ
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void clearOutFlgWithTrackingNumberingUnitCheckNew(Long pickingHId, UpdateInfo updateInfo, TAllocInstH allocInstH, ErrorStatus errSts) {
		...
		// 出力フラグのクリア実行
		clearOutFlgNew(pickingHId, updateInfo, allocInstH, errSts);
	}
	 ...
	 * 帳票の出力状況をクリア
	 */
	private void clearOutFlgNew(Long pickingHId, UpdateInfo updateInfo, TAllocInstH allocInstH, ErrorStatus errSts) {
		...
		// 出庫帳票のステータス更新
		clearOutFlgPickingRNew(pickingR, updateInfo);

		// ミドリ安全様向けカスタマイズ対応  積込リスト出力の更新削除 2024.7.15 張 Del [S]
		// 積込リスト出力の更新
//		if (updateInfo.doClearStwOut) {
//			stwListPrintLogic.clearStwOutFlg(allocInstH, errSts);
//		}
		// ミドリ安全様向けカスタマイズ対応  積込リスト出力の更新削除 2024.7.15 張 Del [E]
	}
	...
	private void clearOutFlgPackingR(TPackingR dbPackingR, UpdateInfo updateInfo) {
		...
		if (updateInfo.doClearTagOut) {
			...
			// ミドリ安全様向けカスタマイズ対応  TagDlUrlの設定削除 2024.7.15 張 Del [S]
//			dbPackingR.setTagDlUrl(null);
			// ミドリ安全様向けカスタマイズ対応  TagDlUrlの設定削除 2024.7.15 張 Del [E]
	 ...
	 * 出庫帳票のスーテタス更新
	 */
	private void clearOutFlgPickingRNew(TPickingR dbPickingR, UpdateInfo updateInfo) {
		boolean isChange = false;
		
		if (updateInfo.doClearTagOut) {
			// 荷札出力クリア
			dbPickingR.setTagOutFlg("0");
			dbPickingR.setTagOutUserId(null);
			dbPickingR.setTagOutDt(null);
			// 梱包明細出力クリア
			dbPickingR.setPllOutFlg("0");
			dbPickingR.setPllOutUserId(null);
			dbPickingR.setPllOutDt(null);
			isChange = true;
		}
	...
	private static class PackingNumInfo {
		...
		private TPackingB packingB;

		public PackingNumInfo(BigDecimal packingNum, long allocInstBId, TPackingB packingB) {
			if (packingNum == null) {
		...
		public TPackingB getPackingB() {
			return packingB;
		}

		public void setPackingB(TPackingB packingB) {
			this.packingB = packingB;
		}
	...
	// ミドリ安全様向けカスタマイズ対応  排他処理追加 2024.7.15 張 Add [S]
	/**
	 * <h2>梱包明細の排他チェック.</h2>
	 * <pre>
	 * 既存梱包明細が編集された場合、排他チェックを行う。
	 *
	 * </pre>
	 * @param packingList 梱包ボディ明細：梱包ボディID
	 * @param errSts エラー時に設定するエラーステータス
	 */
	public void checkPackingLIstUpdate(List<SqlPackingListUpdate> packingList, ErrorStatus errSts) {
		Set<Long> oldBids = packingList.stream().map(entity -> {
			return entity.getOldBid();
		}).collect(Collectors.toSet());
		//排他チェック判断
		boolean isUpdate = false;
		for(Long id : oldBids) {
			TPackingBCB cb = tPackingBBhv.newConditionBean();
			cb.query().setPackingBId_Equal(id);
	...
	// ミドリ安全様向けカスタマイズ対応  排他処理追加 2024.7.15 張 Add [E]
}
