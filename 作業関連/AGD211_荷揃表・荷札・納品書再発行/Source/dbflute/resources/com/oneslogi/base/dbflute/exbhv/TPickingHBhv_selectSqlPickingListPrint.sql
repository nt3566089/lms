...
FROM
  ...
  LEFT JOIN T_SHIPPING_INST_H SIH                 -- 出荷指示ヘッダ
    ...
    AND SIH.DEL_FLG = '0'
  LEFT JOIN T_SHIPPING_INST_B SIB                 -- 出荷指示ボディ
  ...
  LEFT JOIN T_PICKING_H PIH                       -- 出庫ヘッダ
    ...
    AND PIH.DEL_FLG = '0'
  LEFT JOIN T_PICKING_B PIB                       -- 出庫ボディ
    ON PIH.PICKING_H_ID = PIB.PICKING_H_ID
    AND PIB.DEL_FLG = '0'
  LEFT JOIN T_PICKING_R PIR                       -- 出庫帳票
    ON PIB.PICKING_H_ID = PIR.PICKING_H_ID
    AND PIR.DEL_FLG = '0'
  ...
  INNER JOIN M_PRODUCT MPR                        -- 商品マスタ
  ...
  LEFT JOIN (
    ...
    FROM
      ...
      INNER JOIN T_PICKING_H PIH                  -- 出庫ヘッダ
        ON AIH.ALLOC_INST_H_ID = PIH.ALLOC_INST_H_ID
        AND PIH.DEL_FLG = '0'
      INNER JOIN T_PICKING_B PIB                  -- 出庫ボディ
        ON PIH.PICKING_H_ID = PIB.PICKING_H_ID
        AND PIB.DEL_FLG = '0'
