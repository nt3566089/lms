...
/**
 * The abstract condition-query of e_product_price.
 * @author DBFlute(AutoGenerator)
 */
public abstract class AbstractBsEProductPriceCQ extends AbstractConditionQuery {

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public AbstractBsEProductPriceCQ(ConditionQuery referrerQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
     ...
     */
    public HpSLCFunction<EProductPriceCB> scalar_Equal() {
        return xcreateSLCFunction(CK_EQ, EProductPriceCB.class);
    }
     ...
     * @return The object to set up a function. (NotNull)
     */
    public HpSLCFunction<EProductPriceCB> scalar_NotEqual() {
        return xcreateSLCFunction(CK_NES, EProductPriceCB.class);
    }
     ...
     * @return The object to set up a function. (NotNull)
     */
    public HpSLCFunction<EProductPriceCB> scalar_GreaterThan() {
        return xcreateSLCFunction(CK_GT, EProductPriceCB.class);
    }
     ...
     * @return The object to set up a function. (NotNull)
     */
    public HpSLCFunction<EProductPriceCB> scalar_LessThan() {
        return xcreateSLCFunction(CK_LT, EProductPriceCB.class);
    }
     ...
     * @return The object to set up a function. (NotNull)
     */
    public HpSLCFunction<EProductPriceCB> scalar_GreaterEqual() {
        return xcreateSLCFunction(CK_GE, EProductPriceCB.class);
    }
     ...
     * {where FOO &lt;= (select max(BAR) from ...)}
     * <pre>
     * cb.query().<span style="color: #CC4747">scalar_LessEqual()</span>.max(new SubQuery&lt;EProductPriceCB&gt;() {
     *     public void query(EProductPriceCB subCB) {
     ...
     * @return The object to set up a function. (NotNull)
     */
    public HpSLCFunction<EProductPriceCB> scalar_LessEqual() {
        return xcreateSLCFunction(CK_LE, EProductPriceCB.class);
    }

    @SuppressWarnings("unchecked")
    protected <CB extends ConditionBean> void xscalarCondition(String fn, SubQuery<CB> sq, String rd, HpSLCCustomized<CB> cs, ScalarConditionOption op) {
        assertObjectNotNull("subQuery", sq);
        EProductPriceCB cb = xcreateScalarConditionCB(); sq.query((CB)cb);
        String pp = keepScalarCondition(cb.query()); // for saving query-value
        cs.setPartitionByCBean((CB)xcreateScalarConditionPartitionByCB()); // for using partition-by
        registerScalarCondition(fn, cb.query(), pp, rd, cs, op);
    }
    public abstract String keepScalarCondition(EProductPriceCQ sq);

    protected EProductPriceCB xcreateScalarConditionCB() {
        EProductPriceCB cb = newMyCB(); cb.xsetupForScalarCondition(this); return cb;
    }

    protected EProductPriceCB xcreateScalarConditionPartitionByCB() {
        EProductPriceCB cb = newMyCB(); cb.xsetupForScalarConditionPartitionBy(this); return cb;
    }

    // ===================================================================================
    //                                                                       MyselfDerived
    //                                                                       =============
    public void xsmyselfDerive(String fn, SubQuery<EProductPriceCB> sq, String al, DerivedReferrerOption op) {
        assertObjectNotNull("subQuery", sq);
        EProductPriceCB cb = new EProductPriceCB(); cb.xsetupForDerivedReferrer(this);
        lockCall(() -> sq.query(cb)); String pp = keepSpecifyMyselfDerived(cb.query()); String pk = "PRODUCT_PRICE_ID";
        registerSpecifyMyselfDerived(fn, cb.query(), pk, pk, pp, "myselfDerived", al, op);
    }
    public abstract String keepSpecifyMyselfDerived(EProductPriceCQ sq);

    /**
     * Prepare for (Query)MyselfDerived (correlated sub-query).
     * @return The object to set up a function for myself table. (NotNull)
     */
    public HpQDRFunction<EProductPriceCB> myselfDerived() {
        return xcreateQDRFunctionMyselfDerived(EProductPriceCB.class);
    }
    @SuppressWarnings("unchecked")
    protected <CB extends ConditionBean> void xqderiveMyselfDerived(String fn, SubQuery<CB> sq, String rd, Object vl, DerivedReferrerOption op) {
        assertObjectNotNull("subQuery", sq);
        EProductPriceCB cb = new EProductPriceCB(); cb.xsetupForDerivedReferrer(this); sq.query((CB)cb);
        String pk = "PRODUCT_PRICE_ID";
        String sqpp = keepQueryMyselfDerived(cb.query()); // for saving query-value.
        String prpp = keepQueryMyselfDerivedParameter(vl);
        registerQueryMyselfDerived(fn, cb.query(), pk, pk, sqpp, "myselfDerived", rd, vl, prpp, op);
    }
    public abstract String keepQueryMyselfDerived(EProductPriceCQ sq);
     ...
     */
    public void myselfExists(SubQuery<EProductPriceCB> subCBLambda) {
        assertObjectNotNull("subCBLambda", subCBLambda);
        EProductPriceCB cb = new EProductPriceCB(); cb.xsetupForMyselfExists(this);
        lockCall(() -> subCBLambda.query(cb)); String pp = keepMyselfExists(cb.query());
        registerMyselfExists(cb.query(), pp);
    }
    public abstract String keepMyselfExists(EProductPriceCQ sq);
    ...
    //                                                                       Very Internal
    //                                                                       =============
    protected EProductPriceCB newMyCB() {
        return new EProductPriceCB();
    }
    // very internal (for suppressing warn about 'Not Use Import')
    protected String xabUDT() { return Date.class.getName(); }
    protected String xabCQ() { return EProductPriceCQ.class.getName(); }
