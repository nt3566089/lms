...
public class BsTStockCB extends AbstractConditionBean {
    ...
    public static class HpSpecification extends HpAbstractSpecification<TStockCQ> {
        ...
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         * {select max(FOO) from t_picking_record_stock where ...) as FOO_MAX} <br>
         ...
         * }, TPickingRecordStock.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TPickingRecordStockCB, TStockCQ> derivedTPickingRecordStockList() {
            assertDerived("tPickingRecordStockList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TPickingRecordStockCB> sq, TStockCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveTPickingRecordStockList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         ...
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TReceiveLabelCB, TStockCQ> derivedTReceiveLabelList() {
            assertDerived("tReceiveLabelList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TReceiveLabelCB> sq, TStockCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveTReceiveLabelList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         * {select max(FOO) from t_scm_label where ...) as FOO_MAX} <br>
         * t_scm_label by STOCK_ID, named 'TScmLabelList'.
         * <pre>
         * cb.specify().<span style="color: #CC4747">derived${relationMethodIdentityName}()</span>.<span style="color: #CC4747">max</span>(labelCB <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
         *     labelCB.specify().<span style="color: #CC4747">column...</span> <span style="color: #3F7E5E">// derived column by function</span>
         *     labelCB.query().set... <span style="color: #3F7E5E">// referrer condition</span>
         * }, TScmLabel.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TScmLabelCB, TStockCQ> derivedTScmLabelList() {
            assertDerived("tScmLabelList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TScmLabelCB> sq, TStockCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveTScmLabelList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
