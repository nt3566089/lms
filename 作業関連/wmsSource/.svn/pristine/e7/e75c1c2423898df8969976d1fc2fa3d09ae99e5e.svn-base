...
/**
 * The base condition-bean of e_transfer.
 * @author DBFlute(AutoGenerator)
 */
public class BsETransferCB extends AbstractConditionBean {

    // ===================================================================================
    //                                                                           Attribute
    //                                                                           =========
    protected ETransferCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsETransferCB() {
     ...
     * @return this. (NotNull)
     */
    public ETransferCB acceptPK(Long transferId) {
        assertObjectNotNull("transferId", transferId);
        BsETransferCB cb = this;
        cb.query().setTransferId_Equal(transferId);
        return (ETransferCB)this;
    }
     ...
     */
    public ETransferCQ query() {
        assertQueryPurpose(); // assert only when user-public query
        return doGetConditionQuery();
    }

    public ETransferCQ xdfgetConditionQuery() { // public for parameter comment and internal
        return doGetConditionQuery();
    }

    protected ETransferCQ doGetConditionQuery() {
    ...
    protected ETransferCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected ETransferCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        ETransferCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected ETransferCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        return new ETransferCQ(childQuery, sqlClause, aliasName, nestLevel);
    }
     ...
     */
    public void union(UnionQuery<ETransferCB> unionCBLambda) {
        final ETransferCB cb = new ETransferCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final ETransferCQ cq = cb.query(); query().xsetUnionQuery(cq);
    }
     ...
     */
    public void unionAll(UnionQuery<ETransferCB> unionCBLambda) {
        final ETransferCB cb = new ETransferCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final ETransferCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
    ...
    public static class HpSpecification extends HpAbstractSpecification<ETransferCQ> {
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<ETransferCQ> qyCall
        ...
        @Override
        protected String getTableDbName() { return "e_transfer"; }
        /**
         * Prepare for (Specify)MyselfDerived (SubQuery).
         * @return The object to set up a function for myself table. (NotNull)
         */
        public HpSDRFunction<ETransferCB, ETransferCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<ETransferCB> sq, ETransferCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public ETransferCB dreamCruiseCB() {
        ETransferCB cb = new ETransferCB();
        cb.xsetupForDreamCruise((ETransferCB) this);
        return cb;
    }
     ...
     */
    public HpColQyOperand<ETransferCB> columnQuery(final SpecifyQuery<ETransferCB> colCBLambda) {
        return xcreateColQyOperand((rightSp, operand) -> {
            return xcolqy(xcreateColumnQueryCB(), xcreateColumnQueryCB(), colCBLambda, rightSp, operand);
        });
    }

    protected ETransferCB xcreateColumnQueryCB() {
        ETransferCB cb = new ETransferCB();
        cb.xsetupForColumnQuery((ETransferCB)this);
        return cb;
    }
     ...
     */
    public void orScopeQuery(OrQuery<ETransferCB> orCBLambda) {
        xorSQ((ETransferCB)this, orCBLambda);
    }
     ...
     */
    public void orScopeQueryAndPart(AndQuery<ETransferCB> andCBLambda) {
        xorSQAP((ETransferCB)this, andCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final ETransferCB cb;
        if (mainCB != null) {
            cb = (ETransferCB)mainCB;
        } else {
            cb = new ETransferCB();
        }
    ...
    protected String xgetConditionBeanClassNameInternally() { return ETransferCB.class.getName(); }
    protected String xgetConditionQueryClassNameInternally() { return ETransferCQ.class.getName(); }
