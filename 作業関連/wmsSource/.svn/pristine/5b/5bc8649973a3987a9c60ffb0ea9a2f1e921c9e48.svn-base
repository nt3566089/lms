...
public class BsTShippingInstBCB extends AbstractConditionBean {
    ...
    public static class HpSpecification extends HpAbstractSpecification<TShippingInstBCQ> {
         ...
         * @return The information object of specified column. (NotNull)
         */
        public SpecifiedColumn columnUs3fTypeCd() { return doColumn("US3F_TYPE_CD"); }
        /**
        ...
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         * {select max(FOO) from t_shipping_inst_b_print where ...) as FOO_MAX} <br>
         ...
         * }, TShippingInstBPrint.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TShippingInstBPrintCB, TShippingInstBCQ> derivedTShippingInstBPrintList() {
            assertDerived("tShippingInstBPrintList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TShippingInstBPrintCB> sq, TShippingInstBCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveTShippingInstBPrintList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
         * Prepare for (Specify)DerivedReferrer (correlated sub-query). <br>
         * {select max(FOO) from t_shipping_inst_data_transfer where ...) as FOO_MAX} <br>
         ...
         * }, TShippingInstDataTransfer.<span style="color: #CC4747">ALIAS_foo...</span>);
         * </pre>
         * @return The object to set up a function for referrer table. (NotNull)
         */
        public HpSDRFunction<TShippingInstDataTransferCB, TShippingInstBCQ> derivedTShippingInstDataTransferList() {
            assertDerived("tShippingInstDataTransferList"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<TShippingInstDataTransferCB> sq, TShippingInstBCQ cq, String al, DerivedReferrerOption op)
                    -> cq.xsderiveTShippingInstDataTransferList(fn, sq, al, op), _dbmetaProvider);
        }
        /**
