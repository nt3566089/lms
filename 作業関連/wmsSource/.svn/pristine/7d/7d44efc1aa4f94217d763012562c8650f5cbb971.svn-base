	...
	public void insertPacking(WHtShipping wHtShipping, ErrorStatus errSts) {
		...
		if (lstWHtShipping.size() > 0) {
			...
			if (commonLogic.isPickingWorkNo(centerId, targetNo)) {
				...
				tPickingHCB.query().setPickingWorkNo_Equal(wHtShipping.getPickingWorkNo());
				// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.19 talahashi Start
				// 検索対象からステータス55:出荷確定済を対象外 ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
				...
				if(tPickingH == null) {
					//出荷確定済の為、作業を完了できません。
					getErrorManager().add(new ErrorStatus(), WmsMessageConst.HANDY_SHIPPING_CONFIRM_ERROR);
					return;
				}
				// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.19 talahashi End
				...
				// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.20 talahashi Start
				// 検索対象からステータス55:出荷確定済を対象外 ※ 出荷確定後のステータス変化を防ぐため、今回は55のみを対象
				tPackingHCB.query().setPackingStatus_NotEqual_$55();
				...
				// [#18787] 出荷確定済を検品済のステータスに戻る不具合修正 2022.04.20 talahashi End
